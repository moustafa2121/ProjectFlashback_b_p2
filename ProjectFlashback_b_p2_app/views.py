...
from .openAIInterface import chatGPT_inter, dallE_inter
import os, requests, shutil


#base path of the images to be saved
...
def phase2View(request, passedValue, **kwargs):
...
in the Phase2View function, and sends it to the frontend
fetching images is separate from the promptView since images usually take longer to load
...
def fetchImgFromDB(request, storyNumber, stageNumber):
    ...
    #if the image does not exists, it means that this is a new story and the
    ...
    if not storyStage.imgExists:
        ...
        except Exception as _:#something went wrong
            return HttpResponse("failed to prompt image", status=422)
...
"""
called on by the frontend to process a user's input (story prompt)
...
"""
# @delayResponse(30)#dectorator to delay response for testing
def fetchChatGptStory(prompt, currentUser, testingChatGpt=testingChatGpt):
    if testingChatGpt:
        data = getTestData_gpt(baseImgPath)
    else:#prompt chatGPT
        data = chatGPT_inter(prompt)
    
    #save the data to a Story/StoryStage instances and return the Story instance
    return saveStoryFromData(data, prompt, currentUser, baseImgPath, testingChatGpt)

"""
Step 2: Dall-E images
Get images from Dall-E images based on the stages' prompts
...
# @delayResponse(20)#dectorator to delay response for testing
def fetchDallEImg(_, storyNumber, storyStage, testingChatGpt=testingChatGpt):
    #get the image link
    if testingChatGpt:
        imgLink = getTestData_dalle(storyStage.stageNumber)
    else:#prompt Dall-E
       prompt = 'Illustration style: ' \
        + storyStage.illustrationStyle + '. ' \
        + storyStage.stageTitle + ' .' + storyStage.imgPrompt
       imgLink = dallE_inter(prompt)
    ...
    if res.status_code == 200:
        ...
        with open(imgPath,'wb') as f:
            ...
            #set the img to exist=true in the db so it can be fetched
